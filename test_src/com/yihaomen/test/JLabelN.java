...
public class JLabelN extends JFrame {
	...
	// public JLabelN(String WorkshopNo,String LineNo) {

	public JLabelN(String WorkshopNo) {

		super("產線端夜班刷卡程式-V20170706");
		setBounds(12, 84, 1000, 600);
		setResizable(false);
		...
		Shift = new JLabel("夜班刷卡", JLabel.CENTER);
		...
		// final mynewModel myModel = new mynewModel();

		// final JTable mytable = new JTable(myModel);
		// myModel = new mynewModel();
		myModel = new MyNewTableModel(WorkshopNo, "N");
		...
		// tabbedPane.addTab("標籤三", null, panel3, "Third panel");
		...
		// try{
		// User user1 = new User();
		// System.out.println("jtf: " + jtf.getText()+"\t"+"PRIMARY_ITEM_NO:" +
		// textT2_1.getText());
		...
		butT2_2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (!RC_NO.equals("") && RC_NO != "" && RC_NO != null) {
						...
						if (str1 != null) {
							for (Object item : str1) {
								if (((String) item).equals(RC_NO)) {
									isaddItem = false;
									// JOptionPane.showMessageDialog(null,"工單號已存在!","提示",JOptionPane.WARNING_MESSAGE);
									break;
								} else {
									isaddItem = true;
								}
						...
						for (int i = 0; i < countRow; i++) {
							...
							if (State == true) {
								...
								System.out.println("WorkshopNo: " + WorkshopNo);
								System.out.println("RC_NO: " + RC_NO);
								System.out.println("PRIMARY_ITEM_NO: " + PRIMARY_ITEM_NO);
							}
						}
					} else {
					...
					// myModel = new MyNewTableModel(LineNo, "N");
					myModel = new MyNewTableModel(WorkshopNo, "N");
					mytable = new JTable(myModel);
					setTable();
					myScrollPane = new JScrollPane(mytable);
			...
			public void textValueChanged(TextEvent e) {
				...
				if (m.matches() == true) {
					...
					try {
						...
						String ShiftN = "N";
						param.put("Shift", ShiftN);
						System.out.println("CardID1: " + CardID + "WorkshopNo: " + WorkshopNo);
						...
						if (havUser == null) {
							...
							User eif = (User) session.selectOne("selectCountAByCardID", CardID);
							System.out.println("row.getRowsA: " + rows.getRowsa());
							jtextT1_1.setBackground(Color.WHITE);
				...
				if (m.matches() == true) {
					...
					try {
						...
						if (rows.getRowsb() == 0) {
							// /**
							// text2.append();
							System.out.println("row.getRowsB: " + rows.getRowsb());
							User havUser = (User) session.selectOne("selectUserByCardID", CardID);
							if (havUser == null) {
								String swipeDate = DateGet.getDate();
								User selEmp = new User();
								...
								// String SwipeCardTime2 = "2017-06-21
								// 07:30:00";
								String Shift = "N";
								...
								session.update("updateUserByOffNDuty", user1);
								session.commit();
								// text2.append("員工上班刷卡成功！\n------------\n");
							}
		 ...
		 * 
		 *           // text1.setText(""); String pattern = "^[0-9]\\d{9}$";
		 *           Pattern r = Pattern.compile(pattern, Pattern.DOTALL);
		 *           Matcher m = r.matcher(CardID);
		 * 
		 *           String time = DateGet.getTime(); String PROD_LINE_CODE =
		 *           text2.getText(); // 驗證是否為10位整數，是則繼續執行，否則提示 //
		 *           System.out.println(m.matches()); if (m.matches() == true) {
		 *           // String[] a = CardID.split("\\s+"); // Arrays.sort(a); //
		 *           for (int i = 0; i < a.length; i++) { // text2.append(
		 *           "CardID2: " + a[i] + "\n"); // }
		 * 
		 *           try { Map<String, Object> param = new HashMap<String,
		 *           Object>(); param.put("CardID", CardID);
		 *           param.put("PROD_LINE_CODE", PROD_LINE_CODE);
		 *           System.out.println("CardID1: " + CardID + " LineNo: " +
		 *           PROD_LINE_CODE); User rows = (User) session .selectOne(
		 *           "com.yihaomen.mybatis.models.UserMapper.selectCountCByCardID",
		 *           param); System.out.println("param: " + param); //
		 *           通過卡號查詢員工個人信息 // 1、判斷是否今天第一次刷卡 System.out.println(
		 *           "getRowsa: " + rows.getRowsc()); if (rows.getRowsc() == 0)
		 *           { // /** // text2.append(); User eif = (User) session
		 *           .selectOne(
		 *           "com.yihaomen.mybatis.models.UserMapper.selectUserByCardID",
		 *           CardID); // String cardid = eif.getCardID(); String name =
		 *           eif.getName(); String RC_NO = jtf.getText(); String
		 *           PRIMARY_ITEM_NO = text1.getText();
		 * 
		 *           text6.setText("第一次刷卡\n" + "ID: " + eif.getId() + "\nName: "
		 *           + eif.getName() + "\n刷卡時間： " + time + "\n" +
		 *           "員工加班刷卡成功！\n------------\n"); User user1 = new User();
		 *           user1.setCardID(CardID); user1.setName(name); String
		 *           swipeCardTime = time;
		 *           user1.setSwipeCardTime(swipeCardTime);
		 *           user1.setReason("加班"); user1.setRC_NO(RC_NO);
		 *           user1.setPRIMARY_ITEM_NO(PRIMARY_ITEM_NO);
		 *           user1.setPROD_LINE_CODE(PROD_LINE_CODE); session.insert(
		 *           "com.yihaomen.mybatis.models.UserMapper.insertUser",
		 *           user1); session.commit(); // System.out.println("HH!"); //
		 *           text2.append("員工上班刷卡成功！\n------------\n"); //
		 *           System.out.println("呵呵"); } else if (rows.getRowsc() > 0) {
		 *           User eif = (User) session .selectOne(
		 *           "com.yihaomen.mybatis.models.UserMapper.selectUserByCardID",
		 *           CardID); System.out.println("row.getRows: " +
		 *           rows.getRowsc()); text6.append("Name: " + eif.getName() +
		 *           "\n" + "第二次刷卡了！\n\n"); } } finally { session.close(); }
		 *           text9.setText(""); } else {
		 *           System.out.println("無輸入內容或輸入錯誤!"); } // text2.setText("");
		 *           // if(rows.getRowsa) // System.out.println(
		 *           "This is methodA!");
		 * 
		 *           } });
		 */
	...
	public static Object[] getItems() {
		...
		try {
			...
			System.out.println("selectRCNoSize：" + user.size());
			int con = user.size();
			Object[] a = null;
			if (con > 0) {
				a = new Object[con + 1];
